...
def create_donation(request):
	...
	try:
		...
		if request.session['DonationDetails'].get('payment_mode') == 'O':
			...
			if stripe_customer_id is None:
				stripe_customer_id = stripe.Customer.create(
					source=request.POST['stripeToken']).id
		...
		log_donations(user, amount, description, str(request.session['DonationDetails'].get(
			'payment_mode')), email, stripe_donation_subscription_id, donor_name, is_anonymous)
...
def log_donations(user, amount, description, payment_mode, email, stripe_donation_subscription_id, donor_name, is_anonymous):
	...
	Donation.objects.create(
		payment_mode=payment_mode, amount=amount, donor_name=donor_name, donor_email=email, stripe_donation_subscription_id=stripe_donation_subscription_id, user=user, is_anonymous=is_anonymous)
	return
...
def cancel_subscription(request, subscription_type,stripe_subscription_id):
	...
	if subscription_type == 'membership':
		...
		membership_group = Group.objects.get(name=membership_type.name)
		# adding the group to the user
		request.user.groups.remove(membership_group)
...
def last_day_of_year(date_value):
	return datetime.date(year=datetime.date.today().year, month=12, day=31)
